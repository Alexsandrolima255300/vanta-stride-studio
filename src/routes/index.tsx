    ...
    <div className="bg-black text-white">
      <section className="relative isolate min-h-[760px] overflow-hidden bg-black md:min-h-[calc(100vh-0px)]">
        ...
        <div className="relative z-10 mx-auto grid min-h-[760px] max-w-[1500px] items-center px-5 pb-14 pt-28 md:min-h-screen md:grid-cols-[0.78fr_1.22fr] md:px-12 md:pb-10 md:pt-24">
          ...
          >
            ...
            <p className="mt-8 max-w-[510px] text-sm leading-7 text-white/55 md:text-base">
              Engenharia leve, silhuetas limpas e materiais que aguentam o dia inteiro. A nova linguagem VANTA em seis modelos.
            </p>
            <div className="mt-9 flex flex-wrap gap-3">
              <Button asChild size="lg" className="h-12 rounded-none bg-white px-8 text-[10px] font-bold uppercase tracking-[0.25em] text-black hover:bg-white/90">
                <Link to="/catalogo">Comprar agora</Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="h-12 rounded-none border-white/25 bg-black/10 px-8 text-[10px] font-bold uppercase tracking-[0.25em] text-white backdrop-blur-sm hover:bg-white hover:text-black">
                <Link to="/produto/$slug" params={{ slug: "vanta-x1" }}>Ver o X1 <ArrowRight className="ml-2 size-3" /></Link>
              </Button>
            </div>
          </motion.div>
        ...
        <div className="absolute bottom-8 left-1/2 z-20 hidden -translate-x-1/2 flex-col items-center gap-4 md:flex">
          <span className="text-[9px] font-semibold uppercase tracking-[0.45em] text-white/50">Role para explorar</span>
      ...
      <section className="overflow-hidden border-y border-white/10 bg-white py-3 text-black">
        <div className="marquee-track flex w-max gap-10 whitespace-nowrap text-[10px] font-bold uppercase tracking-[0.35em]">
          {Array.from({ length: 8 }).map((_, i) => <span key={i}>Tudo em até 10x sem juros · Frete grátis acima de R$ 399 ·</span>)}
        </div>
      </section>

      <section className="mx-auto max-w-[1400px] px-5 py-24 md:px-10">
        <div className="flex items-end justify-between">
          <h2 className="font-display text-[clamp(2.5rem,5vw,4.5rem)] font-black uppercase tracking-[-0.04em]">Mais vendidos</h2>
          <Link to="/catalogo" className="hidden items-center gap-2 text-xs uppercase tracking-[0.2em] text-white/50 transition-colors hover:text-white md:flex">Ver catálogo <ArrowRight className="size-4" /></Link>
        </div>
      ...
      <section className="border-y border-white/10 bg-[#080808]">
        <div className="mx-auto grid max-w-[1400px] gap-px bg-white/10 md:grid-cols-3">
          {categorias.map((c) => (
            <Link key={c.label} to="/catalogo" className="group bg-[#080808] px-8 py-20 transition-colors hover:bg-[#101010]">
              <p className="text-[10px] uppercase tracking-[0.3em] text-white/35">{c.desc}</p>
              <p className="mt-4 font-display text-5xl font-black uppercase tracking-[-0.04em] text-white transition-colors group-hover:text-orange-400">{c.label}</p>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1400px] px-5 py-24 md:px-10">
        <h2 className="font-display text-[clamp(2.5rem,5vw,4.5rem)] font-black uppercase tracking-[-0.04em]">Coleção nova</h2>
        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {novos.map((p, i) => <ProductCard key={p.slug} product={p} index={i} />)}
        </div>
      </section>
      ...
      <section className="border-t border-white/10">
        <div className="mx-auto flex max-w-[1400px] flex-col gap-8 px-5 py-24 md:flex-row md:items-end md:justify-between md:px-10">
          <div>
            <h2 className="font-display text-[clamp(2.5rem,5vw,4.5rem)] font-black uppercase leading-[0.9] tracking-[-0.04em]">Entre na<br />lista VANTA</h2>
            <p className="mt-5 max-w-sm text-sm leading-6 text-white/45">Drops, restocks e acesso antecipado. Sem spam.</p>
          </div>
          <form className="flex w-full max-w-md gap-3" onSubmit={(e) => { e.preventDefault(); toast.success("Inscrição confirmada", { description: email }); setEmail(""); }}>
            <Input type="email" required placeholder="seu@email.com" value={email} onChange={(e) => setEmail(e.target.value)} className="h-12 rounded-none border-white/15 bg-white/5 text-white placeholder:text-white/25" />
            <Button type="submit" className="h-12 rounded-none bg-white px-6 text-[10px] font-bold uppercase tracking-[0.2em] text-black hover:bg-white/90">Assinar</Button>
          </form>
        </div>
      </section>
    </div>
